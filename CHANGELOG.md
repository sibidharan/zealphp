...
### Changed
- **ZealAPI null-return is now mode-aware — filename-match handlers return empty 200 again** ([#347](https://github.com/sibidharan/zealphp/issues/347) follow-up) — the v0.4.7 fix made ANY `null` + empty + 200 return a `404 {"error":"method_not_found"}`, which wrongly 404'd a **filename-match** handler that legitimately returns the empty set (an infinite-scroll/pagination tail, a poll with no new events — clients read empty-200 as "no more to load"). The two dispatch modes are mutually exclusive (filename-match wins), so the intent is unambiguous from the mode: a **filename-match** handler (`$list`, serves all methods) returning null is an intentional **empty 200** (native-PHP parity — a function with no `return` is null); only a **per-method** handler (`$get`/`$post`/…) that ran and produced nothing yields the 404 envelope (a method with no handler at all already 405s earlier). Escape hatches unchanged (`return '';`, explicit status, `App::apiNullNotFound(false)`). Pinned by the rewritten `tests/Unit/ZealApiNullNotFoundTest.php`.

### Documentation
- **New living guide: [the road to "old PHP just works"](docs/just-works-roadmap.md)** (`/docs/just-works-roadmap`) — the honest scorecard for the coroutine-legacy compatibility runtime: the seven fixed-and-pinned failure classes (with evidence), the two deliberate conditional contracts (class-graph warmup, one-DB-connection-per-coroutine), the nine remaining frontiers ranked by impact with issue links (mysqlnd teardown ext#44/#49, class_alias ext#50, class-static isolation ext#48, …), the permanent boundaries, and how progress is measured. Linked from the docs index + sidebar; surfaced through the docs API allow-lists.

- **Canonical isolation-stage taxonomy** — the stage numbering had drifted into overlapping schemes ("Stage 3.5"/"Stage 4" were components of the one silent-redeclare knob; "Stage 6" was removed machinery; the ext's ARCHITECTURE.md used "Stage 1/2" for implementation GENERATIONS of $GLOBALS; the newer process-setting/reset/exit stages shipped unnumbered). New single source of truth: [`docs/architecture/isolation-stages.md`](docs/architecture/isolation-stages.md) — numbers are append-only, S4/S6 are retired tombstones, components get sub-letters: S1 Superglobals · S2 Globals (incl. request-frame parking, the former "Pass 1b") · S3 Redeclare (S3a opcodes / S3b define intercept / S3c compile CG-swap) · S5 Statics (S5a function / S5b class) · S7 Include-once · S8 Global scope · S9 Process settings (S9a–S9h) · S10 Constants · S11 Request resets (S11a–c) · S12 Exit hook. Adopted across CLAUDE.md, the living docs (`runtime-architecture.md`, `state-isolation-reference.md`, `environment-variables.md`, `running-modern-apps.md`), 10 website guide pages, and the ext repo (`zealphp.c` comments + `ARCHITECTURE.md`, comment-only). Dated `docs/architecture/2026-*` records keep their original numbering (the taxonomy doc carries the alias map).

### Security
- **Session fixation — manager-rotated id no longer reverts to the forged value** ([#371](https://github.com/sibidharan/zealphp/issues/371), CWE-384) — on a forged / strict-mode-rejected `PHPSESSID` the session manager mints a fresh id and records it in `session_params['session_id']`, but the OnRequest superglobal populate then re-parsed the **raw** request cookie and clobbered `$g->cookie[PHPSESSID]` back to the attacker's forged id — so a handler's `session_start()` persisted the session under the forged id (the victim authenticates, the attacker replays the id → takeover). The populate now calls **`App::reassertRotatedSessionId()`** to re-assert the manager's rotated id over the freshly-parsed map (including the first-visit mint). Pinned by `tests/Unit/Session/SessionSecurityFixesTest.php`.
- **`FileSessionHandler` no longer writes sessions to the filesystem root** ([#343](https://github.com/sibidharan/zealphp/issues/343), HIGH) — `open()` computed the `/var/lib/php/sessions` fallback into `$this->savePath` but then **overwrote it back to the raw empty `$savePath`** on the last line (and ran `is_dir()`/`mkdir()` on the empty value), so an empty `session.save_path` wrote `sess_*` to `/` → `Permission denied`, breaking session storage entirely. Now resolves an empty path to `sys_get_temp_dir()` (PHP-FPM parity) and uses the resolved path consistently.

### Fixed
- **Buffer-owning apps no longer serve 200 with a 0-byte body — `flush()`/`ob_flush()`/`ob_end_flush()` semantics corrected** — three layered bugs in the output-buffer overrides, found chasing CodeIgniter 4’s empty welcome page: (1) **an EMPTY `flush()` switched the response to streaming mode and sent headers** — CI4’s `app/Config/Events.php` drains the host’s buffers (`while (ob_get_level() > 0) ob_end_flush();`) before producing output, the premature streaming switch made `ResponseMiddleware` skip body collection, and the 17 KB page echoed later was silently discarded. An empty flush is now a complete no-op (Apache parity: flushing nothing is unobservable). (2) **`ob_end_flush()`/`ob_flush()` above the framework’s capture buffer behaved like `flush()`** instead of native nested semantics — they now pop/pass content into the PARENT buffer (pop-and-echo; the builtin cannot be called from inside its own override), gated by a per-request OB floor (`RequestContext::$_ob_floor`, recorded by `App::executeFile()`). (3) The at-floor pop stays unconditional so buffer-owning drain loops terminate (gating it on “did we stream” span both workers at 100 % CPU). **Validated:** CodeIgniter 4 serves the full page (200/17,244 bytes, burst 84×200, 0 worker deaths — was 200/0 bytes); MyBB/DokuWiki/phpMyAdmin/Kanboard/YOURLS/WordPress gates unchanged; pinned by `tests/Unit/UtilsExtraTest.php` (`testEmptyFlushDoesNotSwitchToStreaming`, `testBufferOwningDrainLoopTerminatesAndBodySurvives`, plus the corrected native-`ob_flush` contract).

- **Session-correctness cluster** ([#369](https://github.com/sibidharan/zealphp/issues/369) [#372](https://github.com/sibidharan/zealphp/issues/372) [#373](https://github.com/sibidharan/zealphp/issues/373) [#374](https://github.com/sibidharan/zealphp/issues/374) [#375](https://github.com/sibidharan/zealphp/issues/375)) — five independent session bugs from a single review pass:
  - **#373** `App::sessionSavePath()` is now honoured by the default file backend + `FileSessionHandler` — `zeal_session_start()` seeds `save_path` from `App::$session_save_path` instead of a hardcoded `/var/lib/php/sessions`, so the documented setter no longer silently writes sessions to the world-writable FPM default.
  - **#369** `session_regenerate_id(true)` / `session_write_close()` reached without a prior `session_start()` no longer fatal — a new `zeal_session_opened_handler()` guarantees the `SessionHandlerInterface` `open()`-before-`write()` contract (`FileSessionHandler`'s typed `$savePath` was accessed before init → uncaught `Error` → 500, breaking the login session-fixation defence).
  - **#374** framework bookkeeping (`__start_time`, `UNIQUE_REQUEST_ID`) no longer leaks into the user `$_SESSION` or the persisted store — it moves to `$g->memo` (both managers + the `utils.php` consumers), and the two legacy keys are stripped on load so a polluted store doesn't resurrect them. `$_SESSION` again contains only the app's own keys (native parity for `count()`/`foreach`/`json_encode`).
  - **#375** `session_set_cookie_params()` positional form adopts PHP 8 keep-current semantics — nullable `$path`/`$domain`/`$secure`/`$httponly`, so a bare `session_set_cookie_params(3600)` no longer silently strips `HttpOnly`/`Secure` from the session cookie; the array form merges over the current params, not hardcoded defaults.
  - **#372** `session_abort()` restores the loaded state instead of emptying it — it decodes the native `key|value;` session file with the session decoder (not raw `unserialize()` → `[]`), so abort no longer wipes the store via the manager's mandatory close.

  Pinned by `tests/Unit/Session/SessionClusterHardeningTest.php` (6 cases). Thanks @Guruprasanth-M for the precise reference-vs-actual reports.

### Added
- **`exit()`/`die()` in coroutines now halts cleanly instead of being swallowed by app catch-blocks** ([ext#47](https://github.com/sibidharan/ext-zealphp/issues/47), Class 2 of the [blocker report](docs/architecture/2026-06-11-coroutine-legacy-blocker-report.md)) — with ext-zealphp 0.3.48+ (the new default pin), a userland exit inside a coroutine throws `ZealPHP\HaltException` instead of `OpenSwoole\ExitException`. The difference is load-bearing: `ExitException` extends `\Exception`, so the ubiquitous legacy-router idiom `try { header('Location: …'); exit; } catch (\Exception $e)` caught the *normal* exit and turned it into a 500 (`"openswoole exit"`) — FreshRSS on every redirecting POST, DokuWiki's fatal-handler swallow, CodeIgniter 4's 0-byte bodies. `HaltException` extends `\Error` (deliberately, since #194) so those catch-blocks can't grab it, and the framework's halt-aware sites flush the buffered output as the body. The exit argument rides `HaltException::$status`/`getStatus()` with the established mapping (string → appended to the body, mod_php parity; int 100–599 → HTTP status; bare exit → unchanged). New knob **`App::hookExit(?bool)`** / `App::$hook_exit` (null = follow the coroutine scheduler at `run()`; env kill-switch `ZEALPHP_EXIT_HOOK_DISABLE=1`); the boot wiring warms `HaltException` pre-fork (the ext deliberately never autoloads from an exit site) and enables `zealphp_exit_hook(true)`. Without the ext the knob is a no-op and OpenSwoole's `ExitException` path (already handled at every dispatch site) continues. Also fixes a latent shadowing bug at the two exit-buffer-collapse sites: unqualified `ob_end_flush()` resolved to the `\ZealPHP\ob_end_flush()` streaming shim, which *discards* the buffer instead of stacking it down — an app's own `ob_start()` content was silently lost on exit. Pinned by `tests/Unit/ExitHaltContractTest.php` (8 cases: status carrier, render + API consumption, nested-OB collapse) and ext `tests/060`/`061`.

### Fixed
- **Session mutations after request 1 are no longer lost in coroutine-legacy — the dominant login blocker** ([#379](https://github.com/sibidharan/zealphp/issues/379), Class 7 of the [blocker report](docs/architecture/2026-06-11-coroutine-legacy-blocker-report.md)) — `zeal_session_start()` in Mode 4 used to bind `$_SESSION = &$g->session`; ext-zealphp's per-coroutine superglobal restore **severed that reference** on the first post-bind yield (the restore `ZVAL_COPY`s a plain array into the `$GLOBALS['_SESSION']` bucket, replacing the `IS_REFERENCE` wrapper). From then on user writes landed in the live `$GLOBALS['_SESSION']` while `$g->session` kept the stale load-time copy — and `zeal_session_write_close()` persisted the stale copy, so **every session mutation after request 1 was silently dropped** (probe: requests ran `prev=1 now=2` forever with the file frozen at `n=1`). In the wild: **phpMyAdmin**'s CSRF token never round-tripped (infinite login loop), **TinyFileManager**'s post-yield login write vanished, **Grav**'s authenticated reads 500'd after a successful login write. Fix: Mode 4 now uses the same **slot-detach alias design as the other six superglobals** (populate `$GLOBALS['_SESSION']`, `unset($g->session)` so the typed slot proxies the live store via the v0.4.8 `__get`/`__set`/`__isset` machinery — ONE store, isolated per coroutine by the ext's proven Stage-1 path), and `write_close()` reads `$GLOBALS['_SESSION']` in every superglobals mode. **Validated on the rig:** the session counter runs 1→2→3→4→5 with the file tracking every write (was frozen at 1), and **TinyFileManager logs in end-to-end** (authenticated listing + persistent revisit; was grade-D for exactly this). Pinned by `tests/Unit/SessionLiveStoreTest.php` (slot-detach + live-store persistence in Mode 4/Mixed + coroutine-mode typed-slot contract).
- **ZealAPI endpoint files can no longer clobber dispatcher locals** ([#376](https://github.com/sibidharan/zealphp/issues/376)) — `processApi()` used to `include` the handler file directly in its own scope, so the filename-handler convention (`${basename(__FILE__, '.php')} = function …`) let any endpoint named after a dispatcher local overwrite it. First casualty: `request.php` set `$request = Closure`, and the Apache-parity `'/api'.$module.'/'.$request.'.php'` concat fataled with `Object of class Closure could not be converted to string` → 500 on every call (hit in production: labs-dashboard-web `api/admin/ssl/request.php`, `api/learn/syllabus/request.php` — fine under Apache, broken on ZealPHP origins; mode-independent, plain include scoping). The include now runs in an **isolated scope** (a non-static closure, so top-level `$this->…` in endpoint files stays bound) and `$get`/`$post`/`$middleware`/`${$func}` resolve from the returned `get_defined_vars()` only — **the reserved-name list is zero**: `request.php`, `module.php`, `func.php`, a stray `$g = …`, etc. are all safe. Pinned by `tests/Unit/ZealApiScopeIsolationTest.php` (the verbatim repro, a second local, a non-Closure `$g` clobber, and the two BC contracts: top-level `$this` binding + per-method dispatch).
...
- **Concurrent-burst worker SIGSEGV (`zend_mm_heap corrupted`) in coroutine-legacy is fixed — ext#52 closed, default pin bumped to ext-zealphp v0.3.49** ([ext#52](https://github.com/sibidharan/ext-zealphp/issues/52), §2 of the [revalidation blocker report](docs/architecture/2026-06-11-revalidation-blocker-report.md)) — ASAN root-caused it to something different from the suspected concurrent-compile race: the engine's `zend_attach_symbol_table`/`zend_detach_symbol_table`/leave-reattach protocol **moves** values between frame CVs with no refcount change, assuming a single flow of control. Two concurrent Stage-8 (`globalScopeInclude`) requests sharing `EG(symbol_table)` hijacked each other's `IS_INDIRECT` buckets — the loser's next write to its stale CV over-freed a payload it no longer owned (10/10 ASAN reports pinned DokuWiki's top-level `foreach` var `$config_group` in `inc/init.php`; the freed payload was an op_array literal string, surfacing later as the heap-corrupted SIGSEGV at an unrelated `require_once`). ext 0.3.47's parking covered **objects only** — its "scalars rely on the frame" assumption was exactly the bug; strings/arrays/refs die the same way. ext 0.3.49 parks ALL request-frame globals per yield, severs the bucket to a plain `NULL` at park (a peer's attach can only ever move a NULL), gates the frame test on the CV **slot** pointer pre-REF-deref (`global $x`-bound slots were silently exempt — the residual `zend_reference` UAF), writes restores **through** global-bound references instead of clobbering the wrapper, and scrubs dangling INDIRECT buckets on the bailout path. **Validated on the rig (84-req 6-way bursts):** DokuWiki 15–28 deaths → **0** (84×200), phpMyAdmin 13–17 → **0**, Kanboard 0–16 → **0**, CodeIgniter4 crash → **0**; ASAN 0 reports / 0 worker exits; Valgrind 0 errors; ext phpt 42/42; new ext `tests/062` pins the contract **bidirectionally** (on 0.3.48 a peer's `global`-ref write leaks into the other request's value and one response is lost; on 0.3.49 both exact). Per-yield cost: one frame-chain walk + O(1) pointer-range checks per key, frames-outer O(1)-lookup restore — runs **only** in coroutine-legacy; `MODE_COROUTINE` hot paths execute none of it.


## [0.4.8] - 2026-06-10

### Fixed
- **`isset($g->server)` now truthful on proxied superglobal slots — the real ext#42 residual** ([ext#42](https://github.com/sibidharan/ext-zealphp/issues/42)) — `RequestContext` gained `__isset()`/`__unset()`. Without `__isset`, PHP reported `isset($g->server) === false` whenever the typed slot was unset-and-proxied (coroutine-legacy's per-request populate, the #346 Apache bridge) even though `__get` returned the fully-populated `$_SERVER` — so (a) every `$g->server['HTTP_HOST'] ?? $fallback` read silently took the fallback (`??` consults `__isset` before `__get` on magic properties), and (b) app-level defensive code reacting to the false negative (`if (!isset($g->server)) { $g->server = []; }`) wiped the live `$GLOBALS['_SERVER']` through `__set` for the rest of the request. Root-caused live on labs-dashboard-web: its `logit()` null-safety guard emptied `$_SERVER` (23 keys → 0) between dispatch and render on **every** page, which is what presented as "HTTP_HOST/DOCUMENT_ROOT empty at render depth" — the request's own coroutine, no child coroutine involved. `__isset` mirrors `isset($_X)` for the seven proxied names (Apache parity — `isset($g->session)` stays false until `session_start()`, preserving `zeal_session_status()`'s inactive-session signal) and `isset($GLOBALS[$key])` for legacy custom keys; coroutine mode reports `false` (an unset typed slot IS "not set"). `__unset` is a deliberate no-op for the proxied names — the framework itself uses `unset($g->server)` as "detach the slot", and escalating a re-detach into `unset($GLOBALS['_SERVER'])` would be the same wipe class — while custom keys keep `__set` symmetry. Pinned by `tests/Unit/RequestContextIssetTest.php` (9 cases, incl. the in-the-wild defensive-guard pattern and the bridge re-entry hazard).


## [0.4.7] - 2026-06-10

### Added
- **`App::go()` + automatic request-context inheritance for child coroutines** ([ext#42](https://github.com/sibidharan/ext-zealphp/issues/42)) — a `go()` child (and every `App::parallel()` / `App::parallelLimit()` task) now inherits the spawning request's `$g` and live superglobals. `RequestContext::instance()` walks the parent-coroutine chain (`Coroutine::getPcid`, depth-capped) so a child shares the request's `$g` exactly like nested includes do in plain PHP; `App::go($fn)` is the request-aware `go()`; `App::adoptRequestContext()` is the manual hook for raw `go()`. Rides ext-zealphp 0.3.43's `zealphp_superglobals_adopt()` (the child gets its OWN superglobal snapshot lane — first-yield capture = inheritance without stealing the parent). Previously a child minted a fresh empty `RequestContext` and read empty/foreign `$_SERVER` at deep render time. Verified 0/300 anomalies at 30-way concurrency.
- **Fatal→500 guard** ([#338](https://github.com/sibidharan/zealphp/issues/338)) — a worker-killing fatal mid-request (E_COMPILE_ERROR / E_ERROR / E_PARSE / E_CORE_ERROR) now answers every in-flight connection with a minimal **HTTP 500** (mod_php parity) instead of leaving clients to time out with HTTP 000, and surfaces the fatal (message + file:line) in the PHP error log. Works in every lifecycle mode (validated E2E in coroutine and coroutine-legacy). Test endpoint `/demo/fatal500` gated behind `ZEALPHP_FATAL_TEST=1`; pinned by `tests/Integration/FatalGuardTest.php`.

### Fixed
- **ext-zealphp pin bumped to v0.3.46** — ships the full coroutine-legacy correctness batch from this cycle:
  - **[ext#37](https://github.com/sibidharan/ext-zealphp/issues/37)** (0.3.41) — request-coroutine claim-set gate on ALL per-request-state stages: a fire-and-forget `go()` child or service coroutine can no longer steal/wipe the live request's `$GLOBALS`/`define()`s/statics/ini.
  - **[ext#40](https://github.com/sibidharan/ext-zealphp/issues/40) / [ext#41](https://github.com/sibidharan/ext-zealphp/issues/41)** (0.3.42) — `$g->server` / `$_SERVER` no longer go empty/contaminated after a coroutine yield; the restore gate keys on the request-coro ptr-set, fixing the `PHP_SELF=`/broken-redirect class.
  - **[ext#42](https://github.com/sibidharan/ext-zealphp/issues/42)** (0.3.43) — child-coroutine superglobal lane (`zealphp_superglobals_adopt()`); see Added above.
  - **(0.3.44)** — per-request `define()` constant leak fixed (deferred-orphan drain keyed by cid; valgrind-proven bidirectionally).
  - **(0.3.45)** — per-coroutine `date_default_timezone_set()` / `mb_internal_encoding()` / `libxml_use_internal_errors()` isolation (auto-on in coroutine-legacy via `App::coroutineTimezoneIsolation()`/`coroutineMbencIsolation()`/`coroutineLibxmlIsolation()`), plus lane-symmetry/close-gate fixes on all process-setting stages. Closes a 179/250-leak class at 49-way concurrency.
  - **[ext#44](https://github.com/sibidharan/ext-zealphp/issues/44)** (0.3.46) — **mysqlnd vio `orig_path` allocator-consistency shim**: the long-standing coroutine-legacy WordPress teardown `zend_mm_heap corrupted` is eliminated in steady state (a persistent-keyed connect malloc'd `orig_path` while the hooked factory returned an emalloc'd stream → `_efree` on a malloc'd pointer). WordPress now serves public traffic + authenticated wp-admin under concurrency with zero corruption.
- **Cold-autoload duplicate-CE crash** — `App::preloadRequestPathClasses()` now also warms the request/response stack in the **master** (pre-fork), not only in the onWorkerStart coroutine where the warm could yield under HOOK_ALL and let a worker take a request mid-warmup. Killed the `Argument #1 ($request) must be of type ZealPHP\HTTP\Request, ZealPHP\HTTP\Request given` respawn cascade on unmodified WordPress under `ab -c20` (14 → 0).
- **Include-boundary `$g` mutation loss** ([ext#43](https://github.com/sibidharan/ext-zealphp/issues/43)) — `App::rebindRequestInput()` now sources the CURRENT live superglobals, so a route callback's `$g->get`/`$g->post` mutations before `App::include()` are visible inside the included file (fixes the `/profile` infinite-redirect class). Request-derived values remain the empty-array fallback.
- **Apache `$g` superglobal bridge** ([#346](https://github.com/sibidharan/zealphp/issues/346)) — under plain Apache/mod_php (no OpenSwoole), the `RequestContext` singleton now unsets its declared request-input slots so `$g->server`/`$g->get`/`$g->request` bridge to `$_SERVER`/`$_GET`/`$_REQUEST` via `__get` instead of reading the empty typed defaults. Pinned by `tests/Unit/RequestContextApacheBridgeTest.php`.
- **ZealAPI unmatched-method parity** ([#347](https://github.com/sibidharan/zealphp/issues/347)) — a filename-match handler returning `null` with no output/status/streaming now emits `404 {"error":"method_not_found"}` (Apache/WebAPI parity) instead of `200` + empty body. Toggle with `App::apiNullNotFound(false)`; pinned by `tests/Unit/ZealApiNullNotFoundTest.php` (7 cases).

### Documentation
- `docs/compatibility-database.md` — 2026-06-10 real-work re-validation on ext 0.3.46 (WordPress public + authenticated wp-admin, 24-concurrent file uploads, Adminer/TinyFileManager): every app boots/renders/auth with zero crashes in steady state.
- New isolation pin: `tests/Integration/PhpInputIsolationTest.php` (php://input per-request body under coroutine concurrency).
...
- **coroutine-legacy session writes no longer vanish via the async-log runner's stale snapshot — ext-zealphp 0.3.37's restore-side ownership gate (ext-zealphp#32).** 0.3.36's ownership gating covered the SAVE side (the go()-child steal); #32 was the RESTORE-side sibling: the framework's long-lived async-log coroutine legitimately accumulates an EMPTY superglobal snapshot (a yield taken while nobody owned the live state), and its later resumes restored it unconditionally — when such a resume landed mid-request it wrote empty arrays over the live `$_SERVER`/`$_SESSION` and `write_close` persisted `[]` over the session file (PHP-8.4-scheduling-dependent: coroutine-legacy counters ran `1→2→1,1` with the file emptied; 8.3 worked by luck). The restore is now gated symmetrically (a snapshot only lands when nobody owns the live state or its recorded owner is the holder), with stale-owner release on coroutine close and request-end teardown. Validated on the previously-failing shape (PHP 8.4 rig, un-instrumented app): coroutine-legacy session counters **1→2→3→4→5→6**, three fresh-boot batches deterministic; bare Mode 4 unchanged; 53/53 ext phpt incl. the new `tests/054` stress (2 cycling service coroutines × 30 owner requests × fire-and-forget children → 0 corruptions). Default ext pin bumped to **v0.3.37**. The ext also ships `ZEALPHP_SG_DEBUG=1` (stderr trace of every superglobal save/skip/clear/restore decision) — the instrument that pinned both bugs.
