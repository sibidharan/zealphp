...
| **Cross-node messaging** | `Store::publish($ch, $payload)` + `App::onPubSub($ch, $handler)` for fire-and-forget pub/sub (cross-worker AND cross-host). `Store::publishReliable($stream, $payload)` + `App::onReliableMessage($stream, $handler)` for Streams-backed at-least-once delivery via consumer groups. The cross-server WebSocket routing pattern (owner-of-fd pushes; Redis routes to owner) lights up end-to-end. **Production note:** for pub/sub subscribers under load, set `ZEALPHP_REDIS_PREFER=predis` (or pass `['prefer' => Store::PREFER_PREDIS]`) until the phpredis SUBSCRIBE+HOOK_ALL spike has been benched in your environment — predis is validated, phpredis is not yet. Hot CRUD paths are unaffected. See [`/store#pubsub`](https://php.zeal.ninja/store#pubsub). |
...
```bash
# New project
composer create-project sibidharan/zealphp-project:^0.2.39 my-project
cd my-project
php app.php
...
3. Tag both `zealphp` and `zealphp-project` with the same version:
   ```bash
   git tag -a v0.2.39 -m "Release v0.2.39"
   git push origin master && git push origin v0.2.39
   ```
