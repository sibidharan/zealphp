...
docker_setup() {
    ...
    echo -e "${YELLOW}Installing ext-zealphp ${ZEALPHP_EXT_VERSION:-v0.3.48} for Docker image.${RESET}"
    ...
    git clone --depth 1 --branch "${ZEALPHP_EXT_VERSION:-v0.3.48}" https://github.com/sibidharan/ext-zealphp.git /tmp/ext-zealphp
...
install_zealphp_ext() {
    ...
    local tmpdir
    tmpdir="$(mktemp -d)"
    git clone --depth 1 --branch "${ZEALPHP_EXT_VERSION:-v0.3.48}" https://github.com/sibidharan/ext-zealphp.git "$tmpdir/ext-zealphp" || {
...
macos_setup() {
    ...
    local tmpdir
    tmpdir="$(mktemp -d)"
    if git clone --depth 1 --branch "${ZEALPHP_EXT_VERSION:-v0.3.48}" https://github.com/sibidharan/ext-zealphp.git "$tmpdir" && \
