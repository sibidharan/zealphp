...
- **DB connections.** HOOK_ALL makes `PDO_MYSQL`/`mysqli` (mysqlnd → `php_stream`) **non-blocking** under coroutines, but hooking ≠ connection-safe: two coroutines sharing one handle interleave wire frames and corrupt the socket. A shared `$pdo`/`$wpdb` still needs **one connection per coroutine** (a pool, mirroring `RedisConnectionPool`). `libpq`-based `PDO_PGSQL`, Oracle/ODBC stay blocking — use OpenSwoole's native `Coroutine\PostgreSQL`. A per-coroutine DB pool is a remaining piece for "unmodified WordPress, concurrent."
- **global-assignment-across-yield — FIXED in ext-zealphp 0.3.22.** `global $x; $x = new Thing()` where the constructor (or RHS) YIELDS (the canonical `global $wpdb; $wpdb = new wpdb()` — `wpdb`'s ctor connects to MySQL → yields under HOOK_ALL) used to lose the write: `zealphp_globals_reset_to_parent()` deleted the live `global $x` IS_REFERENCE slot on yield, detaching the suspended frame's CV, so the post-resume assignment landed in an orphaned ref (→ `wp_set_wpdb_vars`: "assign property field_types on null"). The COW reset now keeps IS_REFERENCE slots and resets their value through `Z_REFVAL` (write-through-ref, like the superglobal restore). Validated 8.3+8.4: repro persists, trust bar 16/16 iso (no regression), ASAN+Valgrind clean. WordPress in coroutine-legacy now boots **past** `$wpdb`; the **remaining** unmodified-WP-concurrent layers are: the per-coroutine DB pool (above), the require_once function/class re-load layers (e.g. `get_network()` undefined — the include-cache vs definition gap), and the compile-hook lost-wakeup on huge single-file compiles (phpMyAdmin/Adminer). Classic WP's correct home stays `legacy-cgi` until those land.
