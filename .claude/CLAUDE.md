...
- **Out of scope (Phase 1)** — tiered Table-L1 + Redis-L2 backend (Phase 2), pub/sub-based L1 invalidation (Phase 3 — spike-gated for phpredis-subscribe-under-HOOK_ALL behaviour), opt-in `$opts['on_error' => 'fallback_table']` per-table degrade hook, Redis Cluster/Sentinel topologies, Redis-backed sessions.
