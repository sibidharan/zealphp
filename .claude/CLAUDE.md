...
> **⚠️ ext-zealphp git tag ≠ binary version — bump BOTH on every release.** The git tag (e.g. `v0.3.25`) and the C version macro `PHP_ZEALPHP_VERSION` (what `php --ri zealphp` / `php -m` report) are **separate** and can drift. A **doc-only re-tag leaves the macro stale**: `v0.3.25` is a docs-only re-tag of `0.3.24`, so the *installed binary reports `0.3.24`* even though you pinned `v0.3.25` — that is **expected, not a broken install** (the 0.3.25 binary is ~276 KB, all the new isolation functions are present; it IS 0.3.24's code). History shows the drift goes both ways: `0.3.8`–`0.3.14` were **header bumps without git tags**; `v0.3.25` is the inverse — a **tag without a header bump**. **RULE when releasing ext-zealphp:** bump `PHP_ZEALPHP_VERSION` in the C header to match the git tag for every tag that ships a binary change, so the reported version matches what downstream pins (`setup.sh` defaults to `${ZEALPHP_EXT_VERSION:-v0.3.59}`; interactive installs go through `pie install zealphp/ext` — pie-first with the tagged source build as fallback, and **no uopz fallback** — ext-zealphp is required). When auditing "is the right ext loaded?", check the **loaded function set** (e.g. `zealphp_reset_request_class_statics`, the 0.3.24/0.3.25 isolation/reset functions), not just the reported version string — the version can lie, the symbol table can't.
...
The knobs `coroutine-legacy` auto-enables (each is also a standalone fluent setter, default **off** so other modes are unaffected). **Stage IDs follow the canonical taxonomy in `docs/architecture/isolation-stages.md`** (S-numbers are append-only; S4/S6 are retired tombstones; components get sub-letters — legacy docs' "Stage 3.5"→S3b, "Stage 4"→S3c, "Pass 1b"→S2 request-frame parking):

| Stage / knob | Setter | What it isolates per coroutine |
|---|---|---|
| **S1** Superglobals | implicit in `coroutine-legacy` | the 7 `$_*` superglobals (IS_REFERENCE-aware, so `$g->get` aliases survive yields) |
| **S2** Globals | `App::coroutineGlobalsIsolation(bool)` | `$GLOBALS` / `global $x` (COW delta vs a once-captured parent baseline) **+ request-frame parking** — every S8 frame CV (all types, ext#52) parks into a per-coroutine registry at yield, bucket severed. Env rollback: `ZEALPHP_GLOBALS_ISOLATION_DISABLE=1` |
| **S3** Redeclare | `App::silentRedeclare(bool)` | conditional `function`/`class` re-declaration → first wins, no `E_COMPILE_ERROR`. Components: **S3a** runtime opcode hook on `ZEND_DECLARE_*`; **S3b** `define()` intercept (silent re-define); **S3c** compile-time CG-table swap + first-wins merge (top-level declarations) |
| **S5** Statics | S5a: `App::coroutineStaticsIsolation(bool)`; S5b implicit | **S5a** function-local `static $x`. **Default-ON in coroutine-legacy** via a `ZEND_BIND_STATIC` touched-set registry — per-yield cost scales with static-*using* functions, not total functions (~µs/yield, decoupled from function count). Opt out: `ZEALPHP_FN_STATICS_DISABLE=1`. Closures/eval excluded (per-instance op_array lifetime). **S5b** class static properties — **v2 (ext 0.3.52): objects included** for post-boot classes (diff-vs-template snapshot, ref-held; live slots re-parked to the boot template on yield; request-end in-coroutine drain; on_close orphan list). Boot classes keep v1 scalar/array behavior. Opt out: `ZEALPHP_CLASS_STATICS_V2_DISABLE=1`. **This closed the ext#44/#49 cold-concurrent worker corruption** — the crash substrate was object graphs in process-shared class statics (WP cold bursts 5/5 deaths→0; phpMyAdmin clean; pinned by ext `tests/065`). **Whole-family opt-out: `App::sharedStatics(true)` / `ZEALPHP_SHARED_STATICS=1`** — statics become WORKER-GLOBAL (vanilla long-process semantics: persistent across requests, shared across concurrent coroutines) while every other stage stays per-request; disables S5a + S5b + the begin/end static resets in one knob. Contract: statics must be request-agnostic (memoisation/config only) — init-once guards gating per-request work (WP `$first_init`) break. For apps that accept that, removes the per-yield statics-walk cost (which scales with touched statics × yields). |
| **S7** Include-once | `App::includeIsolation(bool)` | per-request `require_once`/`include_once` re-execution (opcode hook on the process-wide `EG(included_files)` cache) |
| **S10** Constants *(standalone — NOT auto-enabled by the `coroutine-legacy` preset; opt in explicitly)* | `App::defineIsolation(bool)` | per-request `define()` constants, removed at request end |
| **S9a** CWD (#323) | `App::coroutineCwdIsolation(bool)` | the process working directory — `chdir()` (incl. `executeFile()`'s chdir-to-script-dir) is saved per coroutine on yield (worker baseline re-parked so peers + new coroutines start clean) and restored on resume. **Default-ON in coroutine-legacy** (requires ext-zealphp 0.3.35+'s `zealphp_cwd_isolation`); cost ~one `getcwd`+`strcmp` per yield when on, zero when off. Opt out: `ZEALPHP_CWD_ISOLATION_DISABLE=1` |
| **S9b** locale | `App::coroutineLocaleIsolation(bool)` | `setlocale()` — process-global string/number/date formatting, saved per coroutine on yield (baseline re-parked), restored on resume. **Default-ON in coroutine-legacy** (ext-zealphp 0.3.38+'s `zealphp_locale_isolation`); a boot-time `setlocale()` before `run()` IS the baseline. Opt out: `ZEALPHP_LOCALE_ISOLATION_DISABLE=1` |
| **S9c** umask | `App::coroutineUmaskIsolation(bool)` | `umask()` — process-global file-creation modes; same stage shape (the umask read+re-park is ONE syscall). **Default-ON in coroutine-legacy** (ext-zealphp 0.3.38+'s `zealphp_umask_isolation`). Opt out: `ZEALPHP_UMASK_ISOLATION_DISABLE=1` |
| **S9d** timezone | `App::coroutineTimezoneIsolation(bool)` | `date_default_timezone_set()` — process-global; WordPress-class apps set it per request (measured 179/250 leaks at 49-way concurrency before the stage). ext-zealphp 0.3.45+'s `zealphp_timezone_isolation`, implemented via the engine's own getter/setter pair (version-proof, no module-globals ABI). **Default-ON in coroutine-legacy.** Opt out: `ZEALPHP_TZ_ISOLATION_DISABLE=1` |
| **S9e** mb encoding | `App::coroutineMbencIsolation(bool)` | `mb_internal_encoding()` — process-global (173/250 leaks). ext 0.3.45+'s `zealphp_mbenc_isolation`; auto-refuses when mbstring absent. **Default-ON in coroutine-legacy.** Opt out: `ZEALPHP_MBENC_ISOLATION_DISABLE=1` |
| **S9f** libxml flag | `App::coroutineLibxmlIsolation(bool)` | `libxml_use_internal_errors()` FLAG — process-global (128/250 leaks). ext 0.3.45+'s `zealphp_libxml_isolation`. Fidelity: collected errors survive within a slice (parse + `libxml_get_errors` with no yield between — the dominant pattern) but not across an I/O yield (php-src's own disable semantic frees the list on re-park). **Default-ON in coroutine-legacy.** Opt out: `ZEALPHP_LIBXML_ISOLATION_DISABLE=1` |

(Also in the canonical taxonomy: **S9g** `ini_set()` + **S9h** `putenv()`/`getenv()` — implicit per-coroutine; **S8** global-scope include; **S11** per-request state resets; **S12** exit hook — each documented in its own section below.)

> **Stage-internals invariants (ext 0.3.45, learned driving tz/mbenc to 0 leaks):** every process-setting stage's on_close re-park is **lane-or-owner gated** — a service/timer coroutine (Timer-channel helpers) closing mid-request must NOT re-park the RUNNING request's setting (that was 185/250 of the tz leaks); and saves keep **lane symmetry** — a coroutine that has a snapshot lane keeps save+re-park rights through post-`superglobals_clear()` teardown yields (restore is ungated, so an asymmetric save left the restored value live for the next request). `zealphp_superglobals_clear()` is the normal-path request-end re-park; on_close is the fatal-path backstop. Apply BOTH patterns to any new process-setting stage.
...
- **object-valued `$GLOBALS` — ISOLATED per coroutine as of ext-zealphp 0.3.23.** Scalars/arrays were always isolated; objects were DELIBERATELY excluded (v0.3.12 security review) for a `__destruct`-in-scheduler-callback UAF risk — so an object global re-read after a yield saw another coroutine's object (empirically **22/24 leak**; the trust bar missed it — it probes `$GLOBALS` with a scalar). Two sub-fixes land it safely: (a) **v0.3.22** keeps the live `global $x` IS_REFERENCE slot on yield (write-through-ref via `Z_REFVAL`) so `global $wpdb; $wpdb = new wpdb()` (ctor yields on connect) no longer LOSES the write (→ `wp_set_wpdb_vars` null); (b) **v0.3.23** isolates the OBJECT value itself across yields — its refcount is held by the per-coroutine delta during the request (so the per-yield reset never drops it to zero mid-switch → no UAF), and its FINAL ref is released at request-end by `zealphp_coroutine_globals_request_end()` (called by `CoSessionManager`/`SessionManager` IN COROUTINE CONTEXT, so an I/O `__destruct` — `$wpdb` closing MySQL under HOOK_ALL — can yield). A `cid→Coroutine*` bridge (recorded in `on_yield`) lets that PHP-context drain find its pointer-keyed delta. Validated 8.3+8.4: object matrix 22/24→**0/24**, I/O-`__destruct` 500-concurrent stress **0 errors** (was "API must be called in the coroutine"), trust bar **16/16** (no regression), **ASAN** 500-coro 0 errors, **Valgrind** 0 errors; pinned by `tests/Integration/CoroutineLegacyBehaviorTest.php`. **RESOURCES** (not objects) stay process-shared — a resource handle's lifecycle can't be snapshot/restored. The **remaining** unmodified-WP-concurrent layer is now precisely root-caused (2026-05-30): **Stage 7 re-executing a `require_once`'d inherited-class declaration corrupts its `default_properties_table` → hard SIGSEGV at the next `new`** (see the dedicated "require_once'd inherited classes" section below). This — NOT the DB connection (a red-herring victim of the heap corruption) — is why classic WordPress crashes in coroutine-legacy; `get_network()` undefined is the benign sibling of the same re-execution. The inherited-class corruption is **FIXED in ext-zealphp 0.3.24** (Stage 3c now orphans inherited first-wins losers instead of destroying them — see below). But WordPress *also* trips a SEPARATE cold-boot `mysqlnd`/`libtasn1` connection-teardown heap-overflow (the original crash, reproduces on request 1 where the re-compile path never runs), so the `mysqlnd`/`libtasn1` teardown was LONG SUSPECTED as the heavy/concurrent-WP frontier — **resolved 2026-06-13 (ext 0.3.52)**: the actual substrate was object statics (S5b v2 closes it; WP cold-concurrent 0 deaths) — WP's public + login-auth + comment-write paths now work in coroutine-legacy via the per-request state resets (ext-zealphp 0.3.25; see "Per-request state reset" below). WP is a benchmark; the resets generalise across the `require_once`-legacy class.
- **Closure `static $x`** — excluded from Stage 5 (rarely request-state).
- **`set_error_handler`/`set_exception_handler`, `register_shutdown_function`** — process-global / not specifically isolation-tested under concurrency. (Raw `ob_*` was REMOVED from this list 2026-06-10: the schematic sweep pinned `ob_start`/`ob_get_contents`/`ob_get_level` as correctly per-coroutine — OpenSwoole swaps output-buffer state on context switch — 0 leaks at 66-way concurrency with double yields. Same sweep also pinned `http_response_code()`, `error_reporting()`, and Stage-7 `require_once` re-exec. Reference: ext-zealphp `docs/2026-06-10-coroutine-legacy-schematic-trust-bar.md`.)
- **`pcntl_fork`, `set_time_limit`** — semantics differ under coroutines.
- **mysqlnd vio-stream teardown heap corruption — SHIMMED in ext-zealphp 0.3.46 (ext#44): steady-state ZERO.** Field-level root cause: a persistent-keyed mysqlnd connect makes the generic xport layer malloc `stream->orig_path` (`pestrdup(path, persistent_id ? 1 : 0)`), while the coroutine-hooked transport factory returns an emalloc'd NON-persistent stream → `_efree()` on a malloc'd pointer at teardown. The 0.3.46 shim patches the exported `mysqlnd_mysqlnd_vio_methods` table (copied by value into every vio at creation) to re-pair `orig_path` with the stream's actual allocator right after `open_tcp_or_unix`/`open_pipe` (opt-out `ZEALPHP_MYSQLND_SHIM_DISABLE=1`; inert without mysqlnd). Validated: pre-shim 11–14 worker deaths per WP harness run → 0 in steady state (3 rounds + sustained wrk/ab). **Residual: ~1 cold-boot first-request crash per fresh boot (self-healing respawn) — still open in ext#44**, as is the oddity of WHO persistent-keys WP's connect (no `p:` in DB_HOST). The original characterization note follows. ASAN A/B: with Zend MM ON, every concurrent-WP worker dies with a heap-buffer-overflow at `zend_mm_free_heap ← _efree ← _php_stream_free(streams.c:525) ← mysqlnd_vio_close_stream ← php_mysqli_close ← $wpdb dtor ← zealphp_globals_reset_to_parent` (the request-end object-global drain is just where the last ref drops); with **`USE_ZEND_ALLOC=0`** the SAME binary served **12,287 requests with ZERO ASAN reports** — so the freed pointer is malloc-valid and only Zend MM's metadata lookup explodes: the stream is allocated in one allocator context (persistent/malloc — suspect: OpenSwoole's transport-hook stream factory under HOOK_ALL) and freed via `_efree`. NOT an ext bug (a deferred-dtor experiment changed nothing; the drain is only the trigger site). **⚠️ REFRAMED 2026-06-13 — `USE_ZEND_ALLOC=0` is NOT the fix and is NOT recommended** (canonical: `docs/architecture/2026-06-13-use-zend-alloc-reframe.md`). On the current ext the orig_path mismatch is **fully shimmed** (an in-process `close_stream` classifier logged 0 culprit closes over hundreds of `$wpdb` teardowns under load). The dominant WP-under-load crash is **NOT** an allocator mismatch: it's a **WordPress-specific ~6.4 MB/request memory accumulation** — WP re-bootstraps non-idempotently in a long-lived worker (the same reason FPM recycles via `pm.max_requests`); ZealPHP's own coroutine-legacy isolation is leak-free (~4 KB/req for a trivial app, proven by control) → `memory_limit` OOM → the coroutine-legacy worker-**EXIT** teardown corrupts the heap. **`USE_ZEND_ALLOC=0` only masks it by disabling Zend MM → disabling `memory_limit` enforcement** so the worker never OOM-exits into the corrupting teardown (it never fixed an allocator bug). Crashes are exit-triggered, NOT concurrency: `memory_limit=6G` + no recycle + `wrk -c60 -d40s` = 0 crashes / 0 abnormal exits. **Production fix: `max_request` worker recycle** — already wired (`run(['max_request' => 50])` reaches `$server->set()`; peak worker RSS held at 41 MB vs 1 GB OOM) — or `cgi-pool`/`legacy-cgi`. Remaining rough edge: recycle DURING heavy concurrency trips the same worker-exit teardown (~2 self-healing crashes / 40 s `wrk -c60`) — a targeted ext frontier (graceful coroutine drain + clean isolation teardown before worker exit). ASAN toolchain preserved on the test VM: `/opt/php-asan`, `/wp5app/asan_run*.sh`.
- **Child coroutines (`go()`) — INHERIT the request context as of ext-zealphp 0.3.43 + the #42 framework pair.** Previously a `go()` child was invisible to the superglobal machinery: the save gate correctly stopped it stealing the parent's live state (#332), but that left it with NO snapshot lane — after its first yield it read a peer request's `$_SERVER` or the post-save empties (issue ext#42: deep template renders in a child coroutine saw empty `HTTP_HOST`/`PHP_SELF`/`DOCUMENT_ROOT`). Now: `RequestContext::instance()` walks the parent-coroutine chain (`getPcid`, depth-capped 32) so a child shares the request's `$g`; `zealphp_superglobals_adopt()` (ext 0.3.43) gives a child its OWN superglobal snapshot lane — its first yield CAPTURES the live view (copy-inheritance) without clearing it, then rides the normal restore/save cycle. **Spawn children via `App::go()` / `App::parallel()` / `App::parallelLimit()`** (all auto-adopt); raw `go()` children call `App::adoptRequestContext()` at entry. Pinned by ext `tests/058`; e2e 0/300 anomalies at 30-way concurrency. The process-state stages (constants/ini/statics/$GLOBALS) deliberately still treat children as children — their adoption is a separate design.
...
- On a **WARM** opcache cache hit there is **no recompile**, so silent-redeclare's Stage 3c (CG-table swap during compile) never fires; and because the class is early-bound (not the runtime opcode), Stage 3a's opcode hook never sees it either. opcache's load re-binds the already-present class → `do_bind_class` → `zend_class_redeclaration_error` (`zend_API.c:464`) → **"Cannot redeclare class"** (e.g. WordPress's `WP_Block_Parser_Block` on request 2). **Delayed (`extends`/`implements`) classes go through the runtime opcode and ARE caught** — only simple early-bound classes are the gap.
- Symptom is opcache-SHM-warmth-dependent: a cold worker compiles+caches fresh (Stage 3c works); the next request hits the warm cache → redeclare → worker recycles → repeat. Minimal repro reproduces only once the SHM is warm.
...
- **Stock-PHP fallback (no patch):** keep opcache ON and **opcache-blacklist the application's document-root** so its re-executed files recompile per request, where Stage 3c first-wins works. Recipe: put `<document-root>/` in a file and point `opcache.blacklist_filename` at it (or `opcache.enable_cli=0` if the app needs no opcache). The framework + vendor stay cached either way.
...
- **⚠️ Concurrent Stage-8 requests — the attach-move UAF (ext#52, FIXED in ext-zealphp 0.3.49).** The engine's `zend_attach_symbol_table`/`zend_detach_symbol_table`/leave-reattach protocol **moves** values between frame CVs with NO refcount change, assuming a single flow of control. Two concurrent Stage-8 requests share `EG(symbol_table)`, so a peer's attach hijacked the other request's `IS_INDIRECT` buckets and left a **stale alias** in its CV — whose next overwrite (e.g. the next `foreach` iteration) over-freed a payload it no longer owned → `zend_mm_heap corrupted` worker SIGSEGV under 6-way bursts, sequential traffic clean (ASAN-pinned 10/10 on DokuWiki's top-level `$config_group` in `inc/init.php`; the freed payload was an op_array literal string, so the crash surfaced at an unrelated later alloc — the gdb `require_once` frame was a victim site, NOT the cause; the suspected concurrent-compile race was a misattribution). ext 0.3.47's parking covered **objects only** ("scalars rely on the frame" was exactly the bug). **0.3.49 parks ALL request-frame globals per yield** (strings/arrays/ints/refs), **severs the bucket to plain NULL at park** (invariant: at every suspension point no bucket is INDIRECT into any request frame, so a peer's attach can only move a NULL), gates the frame test on the CV **slot** pointer pre-REF-deref (`global $x`-bound slots were silently exempt — the residual `zend_reference` UAF), restores **through** global-bound references, and scrubs dangling INDIRECT buckets on the `zend_bailout` path. Validated: DokuWiki 15–28 deaths/84-req-burst → **0**, phpMyAdmin 13–17 → **0**, Kanboard 0–16 → **0**, CodeIgniter4 crash → **0**; ASAN 0 reports; Valgrind 0 errors; pinned **bidirectionally** by ext `tests/062` (0.3.48 leaks a peer's `global`-ref write into the other request and loses a response). Per-yield cost: one frame-chain walk + O(1) pointer-range checks per key; coroutine-legacy only — `MODE_COROUTINE` executes none of it.
...
1. Explicit routes defined in `app.php` before `$app->run()` is called (already in `$this->routes` when `run()` captures the baseline — so they're registered FIRST, highest priority)
2. Files from `route/*.php` (included by `run()` via `glob`, appended AFTER the app.php routes)
...
- `RequestHeaderMiddleware` — declarative request-header `set/add/append/unset` written into `$g->server` using the `HTTP_<NAME>` CGI convention, so handlers see the modified headers. Apache `mod_headers RequestHeader` parity (Apache's `edit` regex op is not implemented).
...
- `CsrfMiddleware` — `new CsrfMiddleware(array $exempt = [])`. Per-session CSRF token validated on state-changing methods (token via `X-CSRF-Token` header or form field; stored in the per-request memo); `$exempt` skips listed path prefixes. (Closes roadmap item P1.6.)
- `HealthCheckMiddleware` — `new HealthCheckMiddleware(array $paths = ['/healthz'], ?callable $check = null)`. Short-circuits the configured liveness/readiness paths with a JSON status before routing; optional `$check` callable gates readiness. (Partially closes roadmap item P1.10's `/healthz`.)
- `LocationHeaderMiddleware` — `new LocationHeaderMiddleware(int $correctPort)`. Rewrites the port in an outbound `Location` header to a configured value (e.g. behind a port-translating proxy).
...
| 600–999 | Coerced to **500** like every other out-of-range code (`App::coerceStatusCode()` accepts only `100 ≤ status < 600`). Verified against Apache mod_php 8.4: `http_response_code(600/750/999)` → `HTTP/1.1 500 Internal Server Error` there too — Apache's own status table has no entry for them (#320). The escape hatch Apache honours — a raw `header("HTTP/1.1 600 Custom")` status line passing through verbatim — is a separate tracked parity follow-up. |
...
**Canonical env-var reference:** every `ZEALPHP_*` variable the code reads is documented in `docs/environment-variables.md` — defaults, scope (user / internal / test / build-CI), and `env_flag` bool semantics, with internal IPC vars (`ZEALPHP_REQUEST_CONTEXT`, `ZEALPHP_CWD`, …) walled off so they aren't mistaken for user knobs. The CLI subset is in `docs/cli.md`; cross-linked from `docs/runtime-architecture.md`. When you add a new `getenv('ZEALPHP_…')`, add a row to that doc (a completeness check greps `src/`/`app.php`/`ext/` for `ZEALPHP_[A-Z0-9_]+`).
...
| `utils.php` | Global functions: `coprocess`, `coproc`, `elog`, `zlog`, `access_log`, `response_add_header`, overridden `header`/`setcookie`/`http_response_code` |
