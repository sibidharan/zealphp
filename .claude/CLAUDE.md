...
- **`App::cgiScriptAlias('/cgi-bin', ['mode'=>'proc'])`** — Apache `ScriptAlias` parity: any file under the prefix is executable regardless of extension. **KNOWN LIMITATION:** registers resolution + ExecCGI scope, but URL-level implicit routing is wired per-**extension** only — a ScriptAlias-only setup (no per-extension backend) is reachable via `App::include()` but doesn't yet get an automatic `/{file}.<ext>` route. (Follow-up.)
...
- **Driver caveat:** the predis-side spike (`docs/superpowers/specs/2026-05-23-phase3-pubsub-spike-result.md`) validated SUBSCRIBE yields under HOOK_ALL. phpredis is the preferred driver when `ext-redis` is loaded but its subscribe loop is C-side; the same spike has NOT been re-run with phpredis. CI matrix loads `ext-redis` so the integration tests exercise it, but production deployments running phpredis should bench-test before relying on it. `prefer => 'predis'` in the URL opts forces predis if needed.
- **Live demo:** `route/demo.php` registers `App::onPubSub('demo:pubsub')` + `'demo:pubsub:*'` when Redis backend is active. `/demo/pubsub/publish?channel=&msg=`, `/demo/pubsub/publish-reliable?stream=&msg=`, `/demo/pubsub/log` exercise the public API end-to-end.
