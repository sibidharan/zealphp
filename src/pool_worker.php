...
if (function_exists('zealphp_override') || function_exists('uopz_set_return')) {
    ...
    // mod_php-parity filter_input()/filter_input_array() (#316): pool workers
    // run under the CLI SAPI, whose internal SAPI request tables are EMPTY —
    // native filter_input() returns null even though the per-request dispatch
    // populates $_GET/$_POST/$_COOKIE/$_SERVER. Resolve the INPUT_* bag from
    // the LIVE superglobals (re-read on every call, so each pooled request
    // sees its own input) and delegate to filter_var()/filter_var_array(),
    // mirroring the main worker's \ZealPHP\filter_input override.
    $__pw_filter_bag = function (int $type): array {
        $bag = match ($type) {
            INPUT_GET    => $_GET,
            INPUT_POST   => $_POST,
    ...
    $z_override('filter_input', function (int $type, string $var_name, int $filter = FILTER_DEFAULT, array|int $options = 0) use ($__pw_filter_bag): mixed {
        $bag = $__pw_filter_bag($type);
    ...
    $z_override('filter_input_array', function (int $type, array|int $options = FILTER_DEFAULT, bool $add_empty = true) use ($__pw_filter_bag): array|false|null {
        return filter_var_array($__pw_filter_bag($type), $options, $add_empty);
    }, true);
