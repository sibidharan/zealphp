...
$response-&gt;sse(function ($emit) {
    ...
    <?php App::render('/components/_callout', [
      ...
      'body'    => '<p>Everything above assumes <code>App::mode(App::MODE_COROUTINE)</code> &mdash; the default for new apps. Coroutine mode is what makes streaming cheap: a long-lived stream lives in a coroutine, and the worker can handle hundreds of other requests in parallel. In <code>App::MODE_LEGACY_CGI</code> mode, streams in <code>public/</code> files run through a CGI subprocess (the parent worker forwards stdout chunks via pipes), and the worker is pinned to that one client for the stream&rsquo;s lifetime &mdash; same as Apache+php-fpm. <strong>The CGI bridge exists for unmodified WordPress/Drupal compatibility, not as a recommended streaming path.</strong> Keep Coroutine mode on, and your SSE endpoints scale to thousands of concurrent streams per worker.</p>',
    ]); ?>
