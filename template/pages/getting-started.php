  ...
  <div class="container">
    ...
    <div class="callout warn">
      ...
      <ul class="gs-risk-list">
        <li class="gs-risk-item"><strong>Use <code>$g-&gt;get</code> / <code>$g-&gt;session</code> (recommended)</strong> — works in every mode, no extension needed. With ext-zealphp, <code>$_GET</code> / <code>$_SESSION</code> are also per-coroutine safe (saved/restored on every yield/resume — S1). Without ext-zealphp in coroutine mode, PHP superglobals are process-wide and would leak. See the <a href="/coroutines#state-parity">parity rule</a>. Also audit <code>static</code> variables for cross-request leaks.</li>
...
PHP
    ...
    <div class="callout warn gs-mt-1">
      ...
      The default <code>app.php</code> runs in <strong>coroutine mode</strong> (<code>App::mode(App::MODE_COROUTINE)</code>) — <code>$g-&gt;session</code> / <code>$g-&gt;get</code> / <code>$g-&gt;post</code> are the recommended accessors (per-coroutine, always safe). With ext-zealphp, <code>$_GET</code>/<code>$_SESSION</code> are automatically per-coroutine safe in both modes (saved/restored on every yield/resume).
      <br><br>
      If you're porting a legacy <code>.htaccess</code> + <code>$_*</code> codebase, use one of the lifecycle presets before <code>App::init()</code>: <code>App::mode(App::MODE_LEGACY_CGI)</code> for unmodified WordPress/Drupal (pre-warmed subprocess pool, true per-request isolation), or <code>App::mode(App::MODE_COROUTINE_LEGACY)</code> to run request-style PHP concurrently with per-coroutine isolation of superglobals (S1), <code>$GLOBALS</code> (S2), function-local statics (S5a), and <code>require_once</code> re-execution (S7) (requires ext-zealphp; per-request <code>define()</code> isolation (S10) is a separate opt-in via <code>App::defineIsolation(true)</code>). The raw <code>App::superglobals(true)</code> flag was the legacy way these presets were configured. See <a href="/coroutines#lifecycle-modes">Lifecycle modes</a> and the <a href="/legacy-apps">Legacy apps</a> page for the full migration matrix.
    </div>
