      ...
      <div class="why-engine-grid">
        ...
        <div class="why-engine-col why-engine-col-harness">
          ...
          <ul class="why-engine-list">
            ...
            <li><strong>CGI worker bridge</strong> &mdash; <code>cgiMode('pool' | 'proc' | 'fork' | 'fcgi')</code> dispatches legacy <code>public/*.php</code> files with true global-scope isolation (<code>'pool'</code> is the default: a pre-spawned warm worker pool that keeps the interpreter in memory, ~1&ndash;3&thinsp;ms; <code>'proc'</code> spawns on demand via <code>proc_open</code>, ~30&ndash;50&thinsp;ms; <code>'fork'</code> is the <em>experimental</em> Apache MPM prefork runner that forks a fresh child per request at ~1&thinsp;ms, requires <code>pcntl</code>+<code>posix</code>; <code>'fcgi'</code> forwards to an external FastCGI / php-fpm pool) (<a href="/legacy-apps">/legacy-apps</a>)</li>
            ...
            <li><strong>Coroutine-safe superglobals &amp; lifecycle presets</strong> &mdash; <code>App::mode(App::MODE_COROUTINE_LEGACY)</code> runs legacy request-style PHP concurrently: the seven <code>$_*</code> superglobals (S1), <code>$GLOBALS</code> (S2), function-local statics (S5a), and <code>require_once</code> state (S7) all isolate per coroutine via ext-zealphp; per-request <code>define()</code> isolation (S10) is available as a separate opt-in via <code>App::defineIsolation(true)</code>. One-call lifecycle presets: <code>App::mode(App::MODE_COROUTINE)</code> (modern default), <code>App::mode(App::MODE_LEGACY_CGI)</code>, <code>App::mode(App::MODE_MIXED)</code>. (<a href="/coroutines#lifecycle-modes">/coroutines#lifecycle-modes</a>)</li>
          </ul>
        </div>
      </div>
