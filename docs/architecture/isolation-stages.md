...
| **S5** | Statics | **S5a** function-local `static $x` (BIND_STATIC touched-set registry); **S5b** class static properties — v2 (ext 0.3.52) isolates **objects too** for post-boot classes (diff-vs-template snapshot, ref-held; live slots re-parked to the boot template on yield; request-end in-coroutine drain). Boot classes keep v1 scalar/array-only behavior. Opt out: `ZEALPHP_CLASS_STATICS_V2_DISABLE=1` | S5a: `App::coroutineStaticsIsolation(bool)`; S5b: implicit | `ZEALPHP_FN_STATICS_DISABLE=1` (S5a) | `zealphp_fn_statics_isolation()` | 0.3.x |
